package myai.military.toplevel;
...
import myai.military.toplevel.clustering.deterministic.DeterministicForceGenerator;
import myai.military.toplevel.clustering.deterministic.kmeans.MinDistanceBasedKMeansWrapper;
import myai.military.toplevel.clustering.force.Force;
import myai.military.toplevel.clustering.force.simpleforce.SimpleForce;
import myai.military.toplevel.clustering.force.simpleforce.SimpleForceFactory;
import myai.military.toplevel.orders.AttackForceBattleGroupOrder;
import myai.military.toplevel.orders.MoveBattleGroupOrder;
        ...
        for (Force myForce: myForces) {
            ...
            if (!myForce.getOriginalBattleGroup().getUnits().isEmpty()) {
                ...
                if (isFlash) {
                	...
                	for (Force enemyForce: enemyForces) {
                		SimpleForce simpleForce = (SimpleForce) enemyForce;
                		if (simpleForce.getOwner() == spring.getClb().getGame().getMyTeam()) {
	                        continue;
	                    }                		
